...
public class premiumVehicle extends Vehicle implements Serializable {
    ...
    public premiumVehicle(String vehicleID, char vehicleStatus, String vehicleModel, double dailyRate, int odometer, int dist_allow, int serv_read, int last_serv_read, double extra_charge) {
    ...
    public double hireComplete(int odometer) {
        ...
        System.out.println(getLast_serv_read());
        System.out.println(getServ_read());
        if (charge > 0.0) {
            if ((getOdometer() - initial_read - getDist_allow())>0) {
                double final_charge = charge + (((getOdometer() - initial_read - getDist_allow()) * getExtra_charge()));
                if (final_charge < 0) {
    ...
    public String toString() {
        return "Premium Vehicle ID: " + getVehicleID() + "\tDescription: " + getVehicleModel() + "\tStatus: " + getVehicleStatus() + "\tDaily Rate: " + getDailyRate() + "\tOdometer Reading: " + getOdometer();
    }
}
